...
function getIndoorPowerDetourLaneY(
  ...
  if (rects.length === 0) return null
  const clearance = Math.max(14 / zoom, Math.min(...rects.map((rect) => rect.height * 0.45)))
    ...
    if (points.length > 1) {
      const drawFeedConnections = (strokeStyle: string, width: number) => {
        ...
        for (let i = 1; i < points.length; i++) {
          ...
          if (absDx < axisSnapTolerance || absDy < axisSnapTolerance) {
            if (!useOutdoorChaining) {
              const obstacleRects = [prev.cardRect, curr.cardRect].filter(
                (rect): rect is ReceiverCardRect => !!rect,
              )
              if (obstacleRects.length > 0) {
                const directPath = [prev, curr]
                const obstaclePadding = Math.max(2 / zoom, lineWidth * 0.55)
                if (pathIntersectsRects(directPath, obstacleRects, obstaclePadding)) {
                  const detourY = getIndoorPowerDetourLaneY(prev, curr, obstacleRects, zoom, layoutMidY)
                  if (detourY !== null) {
